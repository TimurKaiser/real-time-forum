...
func AddtoDB(w http.ResponseWriter, register []string) {
	...
	hashpass, err := HashPass(register[6])
	if err != nil {
		handle500(w, err)
		return
	}
	...
	verified := true
	_, err = db.Exec("INSERT INTO Users (username, first_name, last_name, email, age, gender, role, password, Messages, Date, Localisation, Statut, Loisirs, Date_naissance, Sexe, Verification_token, Verified) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", register[0], S, register[2], register[3], register[4], register[5], addRole, hashpass, 0, time.Now().Format("2006-01-02 15:04:05"), "", "", "", "", "", mailtoken, verified)
	if err != nil {
		handle500(w, err)
		return
	}
